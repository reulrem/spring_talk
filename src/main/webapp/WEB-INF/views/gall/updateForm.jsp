...
<sec:authorize access="isAuthenticated()">
	...
	<form action="/gall/update/${gall_name}/${board_num }" method="post">
		...
		<input type="text" name="board_title" class="form-control" value="${board.board_title }"><br/>
		<textarea name="board_content" class="form-control" rows="5" placeholder="내용">${board.board_content }</textarea><br/> 
		<input type="hidden" name="${_csrf.parameterName }" value="${_csrf.token }"/>
		<div class="d-flex justify-content-end">
			<input type="submit" id="submitBtn" class="btn btn-outline-secondary" value="수정완료">
		</div>
	</form>
