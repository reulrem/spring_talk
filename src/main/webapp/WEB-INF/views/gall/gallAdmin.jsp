...
<sec:authorize access="isAuthenticated()">
	...
	<div class="form-signin">
		<form action="/gallList" method="post">
			...
			<label for="inputPassword" class="sr-only">커뮤니티명</label>
			<input type="text" name="gall_name" class="form-control" placeholder="커뮤니티명"/><br/>
			
			<label for="inputPassword" class="sr-only">테이블명(gall_?)</label>
			<input type="text" name="table_name" class="form-control" placeholder="테이블명(gall_?)"/><br/>
			
			<input type="hidden" name="${_csrf.parameterName }" value="${_csrf.token }"/>
			<input type="submit" class="form-control" value="리스트 생성">
		</form>
	</div>
...
<footer class="mx-0 py-2 w-100 border-top row justify-content-between">
	...
	<a href="/user/chatList" class="col-2">
