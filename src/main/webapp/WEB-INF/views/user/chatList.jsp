...
<h3 class="col-11 px-0">DM</h3>
<a href="/post/insert" class="col-1 text-left text-black">+</a>
</header>
<div class="container">
		
		
<c:forEach var="user_id" items="${chatList}" varStatus="status">
	<div class='d-flex justify-content-between row p-2 border-bottom' onclick="location.href='/user/noteDetail/${login_id}/${user_id }'">
	<p class='follower m-0 user_id'>${user_id }</p>
	<span class="text-secondary" id="${user_id }_recent"> 최근 메세지</span>
	</div>
</c:forEach>
...
<!-- jquery cdn 코드 -->
    ...
    let login_id = '${login_id}';
	 ...
	 // 해당 아이디와 주고받은 쪽지 중 최신 쪽지 불러오는 함수
	function recent(user_id){
		var note_recipient = login_id;
		var note_sender = user_id;
		 $.ajax({
				type : 'post',
				url : '/user/chatRecent',
				headers : {
	 ...
	 function callRecent(){
		 for(var i = 0; i < $(".container")[0].childElementCount; i++){
			 var user_id = $(".container")[0].children[i].children[0].innerHTML // 아이디
			 recent(user_id);
		 }
	 } callRecent();
	 
	 
	 $(".container")[0].childElementCount
